...
class ReplayBuffer:
    ...
    def store_transition(self, state, action, reward, next_state, done):
        ...
        self.state_memory[idx]      = torch.as_tensor(state, dtype=torch.float32, device=self.input_device)
        self.next_state_memory[idx] = torch.as_tensor(next_state, dtype=torch.float32, device=self.input_device)
